...
class KNN:
        ...
        for presrow in x:
            ...
            for row in self.data:
                ...
                for i, entry in enumerate(row):
                    ...
                    f = pow(abs(entry-presrow[i]), self.p)
                    val += f
    ...
    def k_neighbours(self, x):
        ...
        dist_index = np.argsort(distances, axis=1)
        neighbourIndex = []
        neighbourDist = []
        for i in range(len(dist_index)):
            ...
            for j in range(self.k_neigh):
                def append_ind_dist(presNeighbourId, curr_neight_dist):
                    presNeighbourId.append(dist_index[i][j])
                    presNeighbourDist.append(distances[i][dist_index[i][j]])
                append_ind_dist(presNeighbourId, presNeighbourDist)

            def append_dist_neighbour(neighbourDist, neighbourIndex):
                neighbourDist.append(presNeighbourDist)
                neighbourIndex.append(presNeighbourId)
            append_dist_neighbour(neighbourDist, neighbourIndex)
