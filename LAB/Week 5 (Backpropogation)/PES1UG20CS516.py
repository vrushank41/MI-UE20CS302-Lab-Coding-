...
class Tensor:
    ...
    def grad_add(self, gradients=None):
        ...
        if False:
            pass
        operand1 = self.history[1]
        operand2 = self.history[2]

        operand1.grad=np.zeros_like(operand1.arr)
        operand2.grad=np.zeros_like(operand2.arr)
        if operand1.requires_grad == True: 
            operand1.grad = operand1.grad + np.ones_like(operand1.arr)
        if operand2.requires_grad == True: 
            operand2.grad = operand2.grad + np.ones_like(operand2.arr)
        if gradients is None:
            return (operand1.grad,operand2.grad)
        if operand1.requires_grad == True: 
            operand1.grad = np.multiply(np.ones_like(operand1.arr), gradients)
        if operand2.requires_grad == True: 
            operand2.grad = np.multiply(np.ones_like(operand2.arr), gradients)
    ...
    def grad_matmul(self, gradients=None):
        ...
        if gradients is None:
            if operand1.requires_grad == True:
                operand1.grad = operand1.grad + np.matmul(np.ones_like(operand1.arr), operand2.arr.transpose())
            if operand2.requires_grad == True:
                operand2.grad = operand2.grad + (np.matmul(np.ones_like(operand2.arr), operand1.arr)).transpose()
        else:
            if operand1.requires_grad == True:
                operand1.grad = operand1.grad + np.multiply(np.matmul(np.ones_like(operand1.arr), operand2.arr.transpose()), gradients)
            if operand2.requires_grad == True:
                operand2.grad = operand2.grad + np.multiply(np.matmul(np.ones_like(operand2.arr), operand1.arr).transpose(), gradients)
